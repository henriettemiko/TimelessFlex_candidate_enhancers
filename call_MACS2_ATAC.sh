...
BED_FILES=($OUTPUT_DIR/ATAC/$TIME/mapping/*_nodup_final_1bp.bed)
NUMBER_BED_FILES="${#BED_FILES[@]}"

if [[ $NUMBER_BED_FILES == 1 ]]
then

    NAME=$(basename $BED_FILES _nodup_final_1bp.bed)

    #if there is no replicate call MACS2 with strict value q=0.01
    macs2 callpeak -t "${BED_FILES[@]}" -f BED -g $MACS2_ORGANISM \
        --outdir ${MACS2_DIR} -n ${TIME}_${NAME}_q001 --nomodel --shift -75 \
        --extsize 150 --keep-dup all -q 0.01
...
then
    ...
    macs2 callpeak -t "${BED_FILES[@]}" -f BED -g $MACS2_ORGANISM \
        --outdir ${MACS2_DIR} -n ${TIME}_pooled_p005 --nomodel --shift -75 \
        --extsize 150 --keep-dup all -p 0.05
    ...
    do
        ...
        NAME=$(basename $i _nodup_final_1bp.bed)
        macs2 callpeak -t $i -f BED -g $MACS2_ORGANISM --outdir ${MACS2_DIR} \
            -n ${TIME}_${NAME}_p005 --nomodel --shift -75 --extsize 150 \
            --keep-dup all -p 0.05
